...
import { syncCohortToSupermemory } from "@/lib/supermemory-behavior";
...
export async function POST(request) {
  try {
    ...
    const result = await syncCohortToSupermemory(cohort, { limit });
    return NextResponse.json({
      success: true,
      data: {
        ...result,
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : "Supermemory sync failed.",
      },
      { status: 500 },
    );
  }
